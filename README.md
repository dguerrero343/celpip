...
AI drafts use versioned CELPIP task-design instructions. Task 1 drafts require a generic recipient role, an explicit tone, one communication purpose, exactly three `•` bullet-point objectives, and the 150–200 word instruction. Candidate-facing prompts cannot contain invented personal names, bracketed or parenthetical coaching, response-quality hints, or requirements to provide dates or times. Task 2 drafts vary among balanced choice, opinion, importance, recommendation, and preference formats and require supporting reasons and examples. Structured output validation rejects malformed metadata, invalid task formats, duplicate prompts, and previously used scenario keys before a draft can be saved. Human review and approval remain required.
